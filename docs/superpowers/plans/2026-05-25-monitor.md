...
export function channelRoutes(app: FastifyInstance, db: DB, reg: Registry): void {
  ...
  app.put("/api/channels/:id", async (req, reply) => {
    ...
    reg.getNotifier(existing.notifierType).configSchema.parse(body.config); // type is immutable; validate against the stored type
...
(also add `import { logger } from "../logger.js";` to `server.ts`)
...
export function monitorRoutes(app: FastifyInstance, db: DB, reg: Registry): void {
  ...
  app.put("/api/monitors/:id", async (req, reply) => {
    ...
    if (!existing) return reply.code(404).send({ error: "not found" });
    const body = upsertSchema.parse(req.body);
    reg.getSource(existing.sourceType).configSchema.parse(body.config); // type is immutable; validate against the stored type
